...
def convert(ur_pose):
    ...
    np_pose = tf.quaternion_matrix(np_q)
    
    # position
    np_pose[0][3] = ur_pose[0]
    np_pose[1][3] = ur_pose[1]
    np_pose[2][3] = ur_pose[2]
    return np_pose
...
def inv_kin(p, q_d, i_unit='r', o_unit='r'):
    ...
    Args:
        p: 6D pose vector [x y z rx rx rz].
